...
function inlineMarkdown(value) {
  ...
  let result = escapeHtml(value)
    ...
    .replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+["']([^"']*)["'])?\)/g, (_, label, url, title) => protect(`<a class="link" href="${safeUrl(url)}"${title ? ` title="${escapeHtml(title)}"` : ""}>${label}</a>`))
    .replace(/`([^`]+)`/g, (_, code) => protect(`<code>${code}</code>`))
    .replace(/\[\^([^\]]+)\]/g, (_, id) => `<sup><a id="fnref-${slugify(id)}" class="link" href="#fn-${slugify(id)}">[${escapeHtml(id)}]</a></sup>`)
